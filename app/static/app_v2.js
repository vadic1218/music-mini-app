...
const STORAGE_KEYS = {
  queue: "ksb-mini-app-queue-v2",
  currentTrack: "ksb-mini-app-current-track-v2",
  shuffle: "ksb-mini-app-shuffle-v2",
  repeatMode: "ksb-mini-app-repeat-mode-v2",
};
...
function currentUserId() {
  return Number(state.telegramUser?.id || 0);
}

function showFatalError(message) {
  const text = String(message || "Ошибка интерфейса.");
  if ($("#health-pill")) $("#health-pill").textContent = text;
  if ($("#status-card")) $("#status-card").textContent = `Mini App остановился с ошибкой:\n${text}`;
...
function setStatusText(message, target = "search") {
  ...
  if (target === "library" && $("#library-summary")) {
    $("#library-summary").textContent = text;
    return;
  }
  if (target === "downloads" && $("#downloads-summary")) {
    $("#downloads-summary").textContent = text;
    return;
  }
  if (target === "library-sync" && $("#library-sync-status")) {
    $("#library-sync-status").textContent = text;
    return;
  }
  if ($("#search-summary")) {
...
function renderQueue() {
  ...
  if (!state.queue.length) {
    container.innerHTML = '<div class="lyrics-card empty-state">Очередь пуста. Добавьте треки кнопкой "Слушать следующим" или запустите список из поиска/библиотеки.</div>';
    persistPlayerState();
    return;
  }
...
function prefetchPlaybackUrls(tracks) {
  tracks.slice(0, 8).forEach((track) => {
    ...
    requestPlaybackUrl(track).then((url) => {
      state.playbackCache[key] = url;
    }).catch(() => {});
  });
}
...
function buildQueueFromCollection(track, collectionTracks) {
  ...
  let remainder = [];
  if (state.shuffle) {
    remainder = shuffleArray(state.currentCollection.filter((item) => trackKey(item) !== current));
  } else {
...
async function playNextTrack() {
  ...
  if (!state.queue.length && state.repeatMode === "all" && state.currentCollection.length) {
    const current = state.currentTrack;
    const collection = state.currentCollection;
    if (current) {
      buildQueueFromCollection(current, collection);
    }
  }
...
function updatePlayerUi() {
  ...
  if ($("#player-toggle")) $("#player-toggle").textContent = hasTrack ? (playing ? "Пауза" : "Слушать") : "Слушать";
  ...
  if ($("#player-dock-toggle")) $("#player-dock-toggle").textContent = playing ? "⏸" : "▶";
...
function renderTrackMeta(track) {
  return [track.album || null, track.duration_seconds ? formatDuration(track.duration_seconds) : null, downloadedLabel(track)]
    .filter(Boolean)
    .join(" • ");
}
...
async function loadLibrary(query = "") {
  ...
  $("#library-summary").textContent = query ? `Найдено в библиотеке: ${payload.tracks.length}` : `Всего в библиотеке: ${payload.tracks.length}`;
  container.innerHTML = "";
  if (!payload.tracks.length) {
    container.innerHTML = '<div class="lyrics-card empty-state">Библиотека пока пустая. Синхронизируйте лайки или добавьте трек из поиска.</div>';
    return;
  }
...
async function loadDownloads(query = "") {
  ...
  $("#downloads-summary").textContent = query ? `Найдено среди скачанных: ${payload.tracks.length}` : `Скачанных треков: ${payload.tracks.length}`;
  container.innerHTML = "";
  if (!payload.tracks.length) {
...
async function syncLibraryFromLiked() {
  ...
  state.lastSyncedNewTracks = payload.library_new_tracks || [];
  ...
  try {
    document.querySelectorAll(".tab").forEach((tab) => {
      tab.addEventListener("click", () => {
        ...
        if (tab.dataset.tab === "library") loadLibrary($("#library-query")?.value?.trim() || "").catch((error) => setStatusText(error.message, "library"));
        if (tab.dataset.tab === "downloads") loadDownloads($("#downloads-query")?.value?.trim() || "").catch((error) => setStatusText(error.message, "downloads"));
      });
    });
